...
namespace XerahS.Core.Helpers
{
    ...
    /// Collects multi-monitor layout, DPI scaling, and coordinate system information.
    ...
    public static class CaptureDebugHelper
    {
        ...
        /// <summary>
        /// Formats a resolution list from width/height pairs. Exposed for testing.
        /// </summary>
        /// <param name="resolutions">List of (width, height) tuples</param>
        /// <returns>String in format "WxH+WxH+..."</returns>
        internal static string FormatResolutionList(IEnumerable<(int Width, int Height)> resolutions)
        {
            return string.Join("+", resolutions.Select(r => $"{r.Width}x{r.Height}"));
        }
