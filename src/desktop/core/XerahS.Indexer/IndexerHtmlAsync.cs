...
{
    /// <summary>
    /// Async streaming HTML indexer.
    /// </summary>
    public class IndexerHtmlAsync : IndexerAsync
    {
        public IndexerHtmlAsync(IndexerSettings indexerSettings, IProgress<IndexerProgress>? progress = null, CancellationToken cancellationToken = default)
        ...
        {
            // For now, fall back to legacy indexer but write to file
            // Full streaming implementation can be added later
            var startTime = DateTime.UtcNow;
            
            try
            {
                var legacyIndexer = new IndexerHtml(settings);
            ...
            string folderPath, string outputFilePath, int maxPreviewLines)
        {
            ...
            // For HTML, read first few lines for preview
